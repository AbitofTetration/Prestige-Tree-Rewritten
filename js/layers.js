...
addLayer("hs", {
        ...
        baseAmount() {return player.s.points}, // Get the current amount of baseResource
        type: "normal", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
        exponent: new Decimal(60), // Prestige currency exponent
        gainMult() { // Calculate the multiplier for main currency from bonuses
            mult = new Decimal(1);
			if (hasUpgrade("g", 35) && player.i.buyables[12].gte(2)) mult = mult.times(upgradeEffect("g", 35));
            return mult
		...
		tabFormat: ["main-display",
			...
			["row", [["buyable", 21], ["buyable", 22], ["buyable", 23], ["buyable", 24], ["buyable", 25], ["buyable", 26], ["buyable", 27]]],
			"blank",
		...
		buyables: {
			...
			11: {
				...
				display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id]
                    let display = ("Cost: " + formatWhole(data.cost.hs) + " Hyperspace Energy"+(tmp.nerdMode?" (Formula: (10^(x^0.9)))":"")+"\nCost: "+formatWhole(data.cost.ba)+" Balance Energy"+(tmp.nerdMode?" (Formula: (10^(((x/1.5)^2)*40+360)))":"")+"\n\
...
addLayer("i", {
		...
		buyables: {
			...
			11: {
				...
				cost(x=player[this.layer].buyables[this.id]) { // cost for buying xth buyable, can be an object if there are multiple currencies
                    let cost = { ib: x.pow(1.2).plus(1).floor(), nb: x.pow(1.4).times(2).plus(4).floor() }
					return cost;
                },
				formulas: {
					ib: "x^1.2+1",
			...
			12: {
				...
				cost(x=player[this.layer].buyables[this.id]) { // cost for buying xth buyable, can be an object if there are multiple currencies
                    let cost = { ib: x.pow(1.2).plus(1).floor(), hb: x.pow(1.8).plus(5).floor() }
					return cost;
                },
				formulas: {
					ib: "x^1.2+1",
				...
				displayData() {
					...
					if (amt.gte(3)) disp += "4 new Enhance Upgrades\n";
